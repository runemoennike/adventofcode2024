...
type Game = 
    ...
    member inline this.adx = fst this.A
    member inline this.ady = snd this.A
    member inline this.bdx = fst this.B
    member inline this.bdy = snd this.B
    member inline this.px = fst this.Prize
    member inline this.py = snd this.Prize
...
let (|PrizePosition|) (input: string) : Position = 
    match Regex(@"Prize: X=(\d+), Y=(\d+)").Match(input) with
    | m when m.Success -> (int64 m.Groups[1].Value, int64 m.Groups[2].Value)
    | _ -> failwith "No prize position in input."

let parse (input: string): Game list =
    input.Trim().Split(Environment.NewLine + Environment.NewLine)
    |> List.ofArray
    |> List.map (fun gameStr ->
        {
            A = match gameStr with | ButtonMovement "A" m -> m
            B = match gameStr with | ButtonMovement "B" m -> m 
...
let solutionsSeq (game:Game): int64 seq = seq {
    for (a: int64) in 0L..maxPresses do
        let xRem, yRem = game.px - game.adx * a, game.py - game.ady * a

        if xRem % game.bdx = 0 && yRem % game.bdy = 0 then
            let b, b' = xRem / game.bdx, yRem / game.bdy

            if b = b' then
                yield a * aCost + b * bCost
}

let games = input |> parse

let solutions =
    games
    |> List.map solutionsSeq
    |> List.filter (Seq.isEmpty >> not)
    |> List.map Seq.min

let totalCost = solutions |> List.sum
